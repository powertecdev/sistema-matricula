...
export default function ClassroomsPage() {
  ...
  const load = async () => {
    setLoading(true);
    try { const res = await classroomApi.list(); setClassrooms(res.data.data || []); }
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Nova Turma">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div><label className="block text-sm text-slate-400 mb-1.5">Capacidade Maxima</label><input type="number" className="input-field" placeholder="30" value={form.maxCapacity} onChange={(e) => setForm({ ...form, maxCapacity: e.target.value })} required /></div>
